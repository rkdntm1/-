...
insert into b_delivery(id, status_id, descrim, member_id, store_id, order_contents, req_end_lat, req_end_lng, req_end_addr, req_delivery_tip, req_cooked_time)
	values('10000', sysdate, 0, '요청', 'member2', 'store1', '김밥', 37.48159487027457, 126.8825609998327, '서울특별시 구로구 가리봉동 42-2 가산디지털단지역 1호선', '3000', '20분');

--수행건 데이터 
--한슬이한테 그 주문이 할당됨
-- (id, status_id, descrim, req_id, rider_id) 	req_id는 요청건에 대한 id와 일치시켜야함
insert into b_delivery (id, status_id, descrim, req_id, rider_id)
	values ('10001', sysdate, 1, '수행', '10000', 'rider2')

--수행건 데이터들 중 픽업했을 시 pickup_dt및 status_id='2'로 배송중 상태로 바뀐다.
--현재시간 입력시 DATE형은 SYSDATE를 TIMESTAMP는 SYSTIMESTAMP를 사용하면 된다.
UPDATE b_delivery 
   SET pickup_dt = SYSTIMESTAMP, status_id = '2'
 WHERE req_id = '10000'

 
--수행건 데이터들 중 배송완료했을 시 pickup_dt및 status_id='3로 배송중 상태로 바뀐다.
UPDATE b_delivery 
...
--취소건 데이터 	
insert into b_delivery_status(id, status_id, descrim, req_id, rider_id, pickup_dt)
	values('10000', 5, '취소');		
...
insert into b_delivery_status(id, status, description)
	values('0', 'pending', '배차대기');
insert into b_delivery_status(status_id, status, description)
	values('1', 'matched', '배차완료');
insert into b_delivery_status(status_id, status, description)
	values('2', 'inTransit', '배송중');
insert into b_delivery_status(status_id, status, description)
	values('3', 'completed', '배송완료');
insert into b_delivery_status(status_id, status, description)
	values('4', 'exception', '예외');	
insert into b_delivery_status(status_id, status, description)
	values('5', 'cancel', '취소');	
