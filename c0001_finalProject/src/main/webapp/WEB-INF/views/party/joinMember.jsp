	...
	<div class="card shadow mb-4">
		<div class="card-body">
			<form id="frmMember" method="post" action="/party/joinMember">
				...
				<!-- 알러지 정보 -->
				<c:forEach items="${listCAType}" var="contactAllergyType" varStatus="status">
					<label for="checkbox">${contactAllergyType.allergyType}<input id = "checkbox" type="checkbox" name="listContactAllergyType[${status.index}].allergyType" class="form-control"></label>
					<input type="hidden" name="listContactAllergyType[${status.index}].contactAllergyType" value="${contactAllergyType.contactAllergyType}" class="form-control" readonly>
				</c:forEach>
