...
<html>
	...
	<body>
		...
		<h1>주문 리스트</h1>
			<form action="/board/storeOrderDetail" method="get">
				<input type='hidden' name='${_csrf.parameterName}' value='${_csrf.token}'>
				<input type="hidden" name="storeId" value="${storeId}">
				<button type="submit" class="btn btn-secondary">Delivery Request만들기</button>
			</form>
			<input id='req'>
			<input id='listRiderPriority'>
		</section>
...
$(document).ready(function(){
	...
	var listRiderPriority = '<c:out value="${listRiderPriority}" />';
	$("#req").val(req);
	$("#listRiderPriority").val(listRiderPriority);
});
</script>